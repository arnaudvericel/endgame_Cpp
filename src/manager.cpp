...
void Manager::sort_files(string dust_infile, string disc_infile, vector<string> particle_filenames) {
...
bool Manager::directory_is_present(string dir_name) {
    struct stat buffer;
    if (stat(dir_name.c_str(), &buffer) == 0) { return true; }
    else { return false; }
}

void Manager::create_output_dir(string dir_name) {
    string command = "mkdir " + dir_name;
    system(command.c_str());
}

void Manager::move_file(string file) {
    string command = "mv " + file + " " + run_directory;
    system(command.c_str());
}

void Manager::copy_file(string file) {
